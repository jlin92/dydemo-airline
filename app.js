// AuraAir multi-page site JS (generated from SPA version)


// ----------------------------
// State (persist across pages)
// ----------------------------
const STATE_KEY = 'auraairState_v1';
...
function loadState(){
  try {
    ...
    return {
      ...
      ...s,
      pax: { ...structuredClone(defaultState).pax, ...(s.pax||{}) },
      selectedFare: { ...structuredClone(defaultState).selectedFare, ...(s.selectedFare||{}) }
    };
  } catch(e){
...
// Base path-aware navigation (works on GitHub Pages subpaths)
function go(rel){
...
function doSearch(){
  captureSearchInputs();
  // from home (root) go into /flightsearch/
  go('flightsearch/');
}
...
function getSelectedDest(){
  return loadState().selectedDest || 'Tokyo';
}
...
function refreshResults() {
  var dest = document.getElementById('r-to').value;
  selectedDest = dest;
...
function initResultsPage(){
  ...
  // Fill header and render
  updateResultsPage(s.selectedDest || 'Tokyo');

  // sync controls
  const rTo = document.getElementById('r-to');
  if (rTo){
    // try to set value if option exists; otherwise set as single option
    const val = s.selectedDest || 'Tokyo';
    const exists = Array.from(rTo.options).some(o => o.value === val || o.text.includes(val));
    if (!exists){
      rTo.innerHTML = `<option value="${val}">${val}</option>`;
    }
    rTo.value = val;
  }
  ...
  // (HTML uses normal links for Back)

  // Ensure sort button matches currentSort
  const btnMap = { price: 'Best Price', duration: 'Duration', depart: 'Departure', arrive: 'Arrival' };
  document.querySelectorAll('.sort-btn').forEach(b => {
    const t = (b.textContent||'').trim();
    const is = Object.entries(btnMap).find(([k,v]) => v===t)?.[0];
    if (is && is===currentSort) b.classList.add('active');
...
function refreshResults(){
  const dest = document.getElementById('r-to')?.value || getSelectedDest();
  saveState({ selectedDest: dest });
  updateResultsPage(dest);
}

// Override selectFare: persist state and navigate to booking step 1
function selectFare(card, fareName, price, flightNum, dep, arr, stopsText){
  ...
  // reset extras/seat when a new fare is picked
...
function syncBookingGlobals(){
  const s = loadState();
  bookingStep = getBookingStep() || 1;
  selectedFare = s.selectedFare;
  selectedDest = s.selectedDest;
  pax = s.pax;
  extrasTotal = s.extrasTotal || 0;
  totalPrice = s.totalPrice || 0;
  selectedSeat = s.selectedSeat || null;
}

function updateBookingPage(){
  syncBookingGlobals();
  const dest = selectedDest || 'Tokyo';
  const info = destInfo[dest] || { code: dest.substring(0,3).toUpperCase() };

  const bookingInfoEl = document.getElementById('bookingFlightInfo');
  if (bookingInfoEl){
    bookingInfoEl.textContent = `AuraAir ${selectedFare.flight} · Singapore → ${dest} · ${selectedFare.name}`;
  }

  const skuBooking = 'SIN-' + info.code;
  const sumRouteEl = document.getElementById('sum-route');
  if (sumRouteEl){
    sumRouteEl.textContent = 'SIN → ' + info.code;
    sumRouteEl.dataset.sku = skuBooking;
  }
  const sumFlight = document.getElementById('sum-flight');
  if (sumFlight) sumFlight.textContent = 'AuraAir ' + selectedFare.flight;

  const sumTimes = document.getElementById('sum-times');
  if (sumTimes) sumTimes.textContent = `${selectedFare.depart} → ${selectedFare.arrive} · ${selectedFare.stops || 'Direct'}`;

  const sumFare = document.getElementById('sum-fare');
  if (sumFare) sumFare.textContent = selectedFare.name;
...
function updateSummaryTotals(){
  ...
  totalPrice = base + tax + ex;
  saveState({ totalPrice });
  if (totalEl) totalEl.textContent = 'SGD ' + totalPrice.toLocaleString();
}
...
function applyExtrasFromState(){
  ...
  document.querySelectorAll('.anc-card').forEach(card => {
    const id = card.dataset.ancId;
    const should = id && set.has(id);
    if (should) card.classList.add('selected');
...
function initDates(){
  ...
  const d1 = fmt(dep); const d2 = fmt(ret);
  const depEl = document.getElementById('searchDepart');
  const retEl = document.getElementById('searchReturn');
  if (depEl && !depEl.value) depEl.value = d1;
  if (retEl && !retEl.value) retEl.value = d2;
  saveState({ searchDepart: depEl?.value || d1, searchReturn: retEl?.value || d2 });
}
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if (page === 'booking'){
    ...
    // Configure next button label on step 4
    const nextBtn = document.getElementById('nextBtn');
    if (nextBtn){
      nextBtn.textContent = step < 4 ? 'Continue →' : 'Confirm & Pay';
    }
  }
